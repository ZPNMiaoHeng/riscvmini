...
//   val mtvec = 0x301.U(12.W)               // FIXME - mtvec no write
...
//   io.evec := mtvec + (PRV << 6)             // 0x100 + 0xC0 =  0x1C0 -> Trap from machine-mode ; trap inst
//   io.epc := mepc
...
//   when(!io.stall) {
//     when(io.expt) {                          // TODO - achieve time interrupt
//       mepc := io.pc >> 2 << 2
//       mcause := Mux(
//         iaddrInvalid,
...
//     }.elsewhen(isEret) {                    // FIXME - modify mret
//       PRV := PRV1
//       IE := IE1
//       PRV1 := CSR.PRV_U
//       IE1 := true.B
//     }.elsewhen(wen) {
//       when(csr_addr === CSR.mstatus) {      // FIXME - MPP/MIE/MPIE
//         PRV1 := wdata(5, 4)
//         IE1 := wdata(3)
//         PRV := wdata(2, 1)
//         IE := wdata(0)
//       }
//         .elsewhen(csr_addr === CSR.mip) {    // ok
//           MTIP := wdata(7)
//           MSIP := wdata(3)
//         }
//         .elsewhen(csr_addr === CSR.mie) {    // ok
//           MTIE := wdata(7)
//           MSIE := wdata(3)
//         }
